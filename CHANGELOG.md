...
- 常见使用场景示例：`examples/embedded.rs`、`examples/mmap_zero_copy.rs`、`examples/server_http.rs`、`examples/nng_client.rs`、`examples/best_performance.rs`、`examples/performance_matrix.rs`、`examples/performance_benchmark.sh`、`examples/siftsmall_benchmark.sh`，覆盖嵌入式、mmap 零拷贝、HTTP Server、NNG 二进制协议、最佳性能配置、自动调参 CSV 矩阵、shell 脚本批量性能测试与真实数据集 siftsmall 验证。
